...
\begin{itemize}
  ...
  \item (2)
        ...
        注意：题中$i_1, i_2, \cdots, i_n$不是排列，是各取各的，互不影响。
        ...
        \end{align*}
        $f$是列线性函数，可以把$A$每列展开，
        最终$A$是有限个矩阵（都由$\epsilon_1, \epsilon_2, \cdots, \epsilon_n$作为列向量组合而成的矩阵）
        记为$E_i(i = 1, 2, \cdots, s)$
        相加得到，即
        \begin{align*}
          A = \sum_{i = 1}^s E_i
        \end{align*}
        于是由列线性函数的性质可知
        \begin{align*}
          f(A) = f(\sum_{i = 1}^s E_i) = \sum_{i = 1}^s f(E_i)
        \end{align*}
  \item (3)

        可以先看(4)，通过其中(ii)的描述，就可以表示出想要的函数了。

  \item (4)

        由(2)可知，任意$f \in P(K)$由$n^n$个变量（(2)中描述的矩阵）确定，
        这个$n^n$个矩阵，我们把它排序成$A_1, A_2, \cdots, A_{n^n}$，
        定义一组函数$f_i \in P(K)$：
        \begin{equation*}
          \begin{cases}
            f_i(A_i) = 1 \\
          ...
          \end{cases}
        \end{equation*}
        现在证明这$n^n$个函数是$P(K)$的一组基，为了讨论方便，这组函数记为$(I)$.

        (i)线性无关。

        令
        \begin{align*}
          f = c_1 f_1 + c_2 f_2 + \cdots + c_{n^n} f_{n^n} = 0
        \end{align*}
        以上是一个常量函数，即任意矩阵$A$都是$f(A) = 0$。
        反证法，假设$(I)$不是线性无关的，即存在$c_j \neq 0$，使得上式成立。
        取$A = A_j$，则
        \begin{align*}
          f(A) & = c_1 f_1(A_j) + c_2 f_2(A_j) + \cdots + c_j f_j(A_j) + \cdot + c_{n^n} f_{n^n} \\
               & = 0 + c_j f_j(A_j)                                                              \\
               & = c_j \cdot 1                                                                   \\
               & = c_j                                                                           \\
               & \neq 0
        ...
        令
        \begin{align*}
          g = b_1 f_1 + b_2 f_2 + \cdots + b_{n^n} f_{n^n}
        \end{align*}
        因为对任意$A_i$，都有
        \begin{align*}
          f(A_i) = g(A_i) = b_i
  ...
  \item (5)
        ...
        \begin{itemize}
          \item 一个错误方法；
                ...
                受(1)-(4)的启发: 我们知道反对称的列线性函数$f$，
                因为在不满秩的矩阵上，函数值总是0，于是推出$\epsilon_i, i = 1, 2, \cdots, n$
                在矩阵中不可重复，所以是$1, 2, \cdot, n$的排列组合，即维数是
                $n!$。

                上面方法的问题在于，我们应该只能推出维数$\leq n!$，
                因为反对称函数还有隐含的性质：交换矩阵两行，函数值前后是相反数，
                而不是在这$n!$上相互独立，即维数$< n!$。

          \item 正确方法.

                我们知道行列式（$det$）的定义为：反对称列线性函数 + $det(E) = 1$，
                而反对称函数的在$E$上的值确定了，由第三章命题2.3可知其他矩阵上的值也就确定了，
                于是对任意反对称列线性函数$f$，如果已知$f(E) = a \cdot det(E)$，则这个函数就被唯一确定了，
                综上可得，反对称列线性函数的维数是$1$，$det$是一组基。
...
\begin{itemize}
  \item (1)
        ...
        零元素为零函数$f_0$（即对任意$A \in M_n(K)$），都有$f_0(A) = 0$。\\
        任意$f \in F(K)$，负元素为$-f$。

  \item (2)

        \begin{itemize}
          \item (i) $Q(K)$是$F(K)$的子空间。

                设$A \in M_n(K)$，为
                \begin{align*}
                  A = (\beta_1, \cdots, \cdots, \beta_j + \beta, \cdots, \beta_n)
                \end{align*}
                其中
                \begin{align*}
                  A_1 = (\beta_1, \cdots, \cdots, \beta_j, \cdots, \beta_n) \\
                  A_2 = (\beta_1, \cdots, \cdots, \beta, \cdots, \beta_n)   \\
                  A = A_1 + A_2
                \end{align*}
                ...
                \begin{align*}
                  (q_1 + q_2) (A)
                   & = q_1(A) + q_2(A)                           \\
                   & = q_1(A_1 + A_2) + q_2(A_1 + A_2)           \\
                   & = q_1(A_1) + q_1(A_2) + q_2(A_1) + q_2(A_2) \\
                   & = (q_1 + q_2)(A_1) + (q_1 + q_2)(A_2)
                \end{align*}
  ...
  \item (3)
        ...
        由习题28-(2)可知，
        任意的行线性函数或列线性函数，由$n^n$个下列函数值确定：
        $f(\epsilon_{i_1}, \epsilon_{i_2}, \cdots, \epsilon_{i_n})$
        其中$i_1, i_2, \cdots, i_n$分别取$1, 2, \cdots, n$。

        任意$f \in P(K) \cap Q(K)$，则
        $i_1, i_2, \cdots, i_n$有重复时，
        函数值一定为$0$。因为$\epsilon_{i_1}, \epsilon_{i_2}, \cdots, \epsilon_{i_n}$
        中一共有$n$个1，存在$i_j$重复，则某列的1个数$\geq 2$，此时将有一行的元素全为0，
        由于$f$是行线性函数，所以$f$此时的函数值为0。

        任意$f \in P(K) \cap Q(K)$，
        $i_1, i_2, \cdots, i_{n!}$可以排列组合处$n!$个矩阵，
        我们把它记为$A_1, A_2, \cdots, A_{n!}$，定义一组函数$f_i$：
        \begin{equation*}
          \begin{cases}
            f_i(A_i) = 1 \\
            f_i(A_m) = 0, m \neq i
          \end{cases}
        \end{equation*}
        现在证明这$n!$个函数是$P(K) \cap Q(K)$的一组基，为了讨论方便，这组函数记为$(I)$.

        (i) 线性无关。

        令
        \begin{align*}
          f = c_1 f_1 + c_2 f_2 + \cdots + c_{n!} f_{n!} = 0
        \end{align*}
        以上是一个常量函数，即任意矩阵$A$都是$f(A) = 0$。
        反证法，假设$(I)$不是线性无关的，即存在$c_j \neq 0$，使得上式成立。
        取$A = A_j$，则
        \begin{align*}
          f(A) & = c_1 f_1(A_j) + c_2 f_2(A_j) + \cdots + c_j f_j(A_j) + \cdot + c_{n!} f_{n!} \\
               & = 0 + c_j f_j(A_j)                                                          \\
               & = c_j \cdot 1                                                               \\
               & = c_j                                                                       \\
               & \neq 0
        ...
        (ii)可以表示$P(k) \cap Q(K)$中的任意列线性函数$f$.

        利用习题28-(2)的结论，排除哪些一定等于$0$的函数值，我们只需定义$f$在$n!$个函数值。
        ...
        令
        \begin{align*}
          g = b_1 f_1 + b_2 f_2 + \cdots + b_{n!} f_{n!}
        \end{align*}
        因为对任意$A_i$，都有
        \begin{align*}
          f(A_i) = g(A_i) = b_i
        ...
        综上，$(I)$就是$P(K) \cap Q(K)$的一组基，$P(K) \cap Q(K)$的维数是$n!$.

  \item (4)

        可以利用定理2.1的证明过程，来找到$P(K) + Q(K)$的一组基。
        过程比较显然，证明略。
