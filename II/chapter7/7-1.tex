...
\begin{align*}
  ker\, \mathscr{A}^k = span \{\epsilon_{n - k + 1}, \epsilon_{n - k + 2}, \cdots, \epsilon_n\}
\end{align*}
所以
\begin{align*}
  \{0\} \subset ker \mathscr{A} \subset ker \mathscr{A}^{2} \subset \cdots \subset ker \mathscr{A}^{n - 1} \subset V
\end{align*}
另外，需要证明$\mathscr{A}$的任意不变子空间，都属于以上子空间。

todo

% $\epsilon_1, \epsilon_2, \cdots, \epsilon_n$是一组循环基，故也可表示成
% \begin{align*}
%   e_1 & = \epsilon_n,             \\
%   e_2 & = \mathscr{A} e_1,        \\
%       & \vdots                    \\
%   e_n & = \mathscr{A}^{n - 1} e_1
% \end{align*}
% 设$M$是非零不变子空间，任意$\beta \in M$，都可以被
% $e_1, e_2 \cdots, e_n$线性表示，
% 我们定义函数
% \begin{align*}
%   h(w) = max{i | w = a_1 e_1 + a_2 e_2 + \cdots + a_n e_n}
% \end{align*}
% 我们称$h(w_0), $为向量的高度，我们可以定义上确界为
% \begin{align*}
%   s = \sup \{h(w) | w \in M\}
% \end{align*}
% 因为$W$非空和有上界，由上界定理可知，上确界$s$存在.

% 对$e_s$反复使用$\mathscr{A}$，可以得到
% \begin{align*}
%   \mathscr{A} e_s             & = e_{s + 1} \\
%   \vdots                                    \\
%   \mathscr{A}^{n - s + 1} e_s & = e_n
% \end{align*}
% 由于$M$是非零不变子空间，所以$e_n \in M$，
